...
def slurp_cube(input_file:str, prefix:str, outfile:str, 
    ...
    for name in PARAMS[:-1]:
        ...
        out_dict[name] = np.linspace(
            vparam_dict[name]["min"], vparam_dict[name]["max"], vparam_dict[name]["n"]
        )
    # Survey items
    for key in survey_arrays.keys():
...
def apply_gaussian_prior(lls:np.ndarray,
    ...
    if iparam < 0 or iparam >= NDIMS:
        raise ValueError(
            ...
            NDIMS,
            " dimensions.",
            "Please select iparam between 0 and ",
            NDIMS - 1,
            " not ",
            iparam,
        )
...
def apply_H0_prior(
    ...
    if H0dim < 0 or H0dim >= NDIMS:
        raise ValueError(
            ...
            NDIMS,
            " dimensions.",
            "Please select H0dim between 0 and ",
            NDIMS - 1,
            " not ",
            H0dim,
        )
...
def get_bayesian_data(lls: np.ndarray, plls: np.ndarray = None, pklfile=None):
    ...
    wvectors = (
        []
    )  # holds same as above, but including spectral penalty factor from ASKAP obs
    ...
    for i in np.arange(NDIMS):
        ...
        # Check
        vector *= 1.0 / np.sum(vector)
        vector *= 1.0 / np.abs(uvals[i][1] - uvals[i][0])
        vectors.append(vector)
        if plls is not None:
            wvector *= 1.0 / np.sum(wvector)
            wvector *= 1.0 / np.abs(uvals[i][1] - uvals[i][0])
...
def get_2D_bayesian_data(lls: np.ndarray, plls: np.ndarray = None, pklfile=None):
    ...
    for i in np.arange(NDIMS):
        ...
        for j in np.arange(NDIMS - i - 1) + i + 1:
            ...
            array *= 1.0 / np.sum(array)
            arrays.append(array)
            if plls is not None:
                warray *= 1.0 / np.sum(warray)
...
def do_single_plots(uvals,vectors,wvectors,names,tag:str=None, fig_exten='.png',
    ...
    if dolevels:
        results = np.zeros(
            [len(uvals), 9]
        )  # holds mean and error info for each parameter
    ...
    for i, vals in enumerate(uvals):
        ...
        temp = np.where((vectors[i] > 0.0) & (np.isfinite(vectors[i])))
        ...
        if wvectors is not None:
            wx, wy = interpolate_points(
                vals[temp], wvectors[i][temp], logspline, kind=kind
            )
                ...
                if wvectors is not None:
                    if ik1 != 0:
                        ...
                        plt.plot(
                            ...
                            [ymax, y[ik1]],
                            color="blue",
                            linestyle=styles[iav],
                            alpha=0.5,
                        )
                        if i == 1:
                            t = plt.text(
                                x[ik1] + doff * 0.5,
                                (ymax) + (-3.6 + iav) * 0.2 * ymax,
                                labels[iav],
                                rotation=90,
                                fontsize=12,
                            )
        ...
        if wvectors is not None:
            plt.plot(
                vals[temp], wvectors[i][temp], color="orange", linestyle="", marker="o"
            )
            if dolevels == True:
        ...
        if others is not None:
            if others[i] is not None:
                for io, data in enumerate(others[i]):
                    x, y = interpolate_points(vals, data, logspline, kind=kind)
                    norm = np.sum(y) * (x[1] - x[0])  # integral y dx ~ sum y delta x
                    norm = np.abs(norm)
                    y /= norm
        ...
        if truth is not None:
            plt.plot(
                [truth[i], truth[i]], plt.gca().get_ylim(), color="black", linestyle=":"
            )
            Dx = x[-1] - x[0]
...
def gen_vparams(indices: tuple, vparam_dict: dict):
    ...
    for ss, key in enumerate(vparam_dict.keys()):
        ...
        #
        vals = np.linspace(
            vparam_dict[key]["min"], vparam_dict[key]["max"], vparam_dict[key]["n"]
        )
        #
